...
for image in images:
    ...
    cat['r_half_error'] = cat['r_half_pix'] * cat['source_error'] / (2 * np.abs(cat['segment_flux']) + 1e-10)
    
    band_catalogs[image.band] = cat[['xcentroid', 'ycentroid', 'source_sky', 'source_error', 'r_half_pix', 'r_half_error']].copy()
...
for idx, ref_source in band_catalogs[reference_band].iterrows():
    ...
    r_half_errors = [ref_source['r_half_error']]
    ...
    for band in ['i', 'u', 'v']:  # Other bands
        ...
        if distances.min() < match_radius:
            ...
            r_half_errors.append(cat.loc[closest_idx, 'r_half_error'])
        else:
    ...
    if found_in_all_bands:
        # Use weighted mean for r_half (inverse variance weighting)
        r_half_arr = np.array(r_half_values)
        r_half_err_arr = np.array(r_half_errors)
        weights = 1.0 / (r_half_err_arr**2 + 1e-10)  # Avoid division by zero
        matches['r_half_pix'] = np.sum(r_half_arr * weights) / np.sum(weights)
        matches['r_half_error'] = 1.0 / np.sqrt(np.sum(weights))  # Weighted error
